...
public class ModelArmorTemplateHandlerImpl
	...
	private ModelArmorTemplateConfig _getModelArmorTemplateConfig(
		...
		_putRaiFilter(
			...
			properties.get("raiDangerousLevel"));
		_putRaiFilter(
			raiFilters, RaiFilterType.HARASSMENT,
			properties.get("raiHarassmentLevel"));
		_putRaiFilter(
			raiFilters, RaiFilterType.HATE_SPEECH,
			properties.get("raiHateSpeechLevel"));
		_putRaiFilter(
	...
	private void _putRaiFilter(
		...
		if (Objects.equals(Objects.toString(levelProperty, "none"), "none")) {
			return;
		}

		raiFilters.put(
			raiFilterType,
			_toConfidenceLevel(Objects.toString(levelProperty, "none")));
	}
