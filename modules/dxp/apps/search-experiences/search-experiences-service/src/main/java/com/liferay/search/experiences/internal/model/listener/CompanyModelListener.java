...
public class CompanyModelListener extends BaseModelListener<Company> {
	...
	public void onAfterCreate(Company company) {
		TransactionCommitCallbackUtil.registerCallback(
			() -> {
				try {
					_sxpElementLocalServiceImpl.importSXPElements(company);
				}
	...
	public void onAfterRemove(Company company) {
		try {
			...
			_sxpElementLocalServiceImpl.deleteCompanySXPElements(
				company.getCompanyId());
		}
		catch (PortalException portalException) {
			_log.error(portalException);
	...
	@Reference(
		target = "(component.name=com.liferay.search.experiences.service.impl.SXPElementLocalServiceImpl)"
	)
	private SXPElementLocalService _sxpElementLocalServiceImpl;
